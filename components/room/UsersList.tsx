...
type PresenceUser = {
  username: string;
  lastSeen: string; // ISO string from your API
  isLive: boolean;
};
  ...
  return (
    <ul className="p-3 space-y-2">
      {users.map((u) => {
        ...
        const last = isNaN(lastDate.getTime()) ? null : lastDate.toLocaleTimeString();
        ...
        return (
          ...
          >
            ...
            <span className="ml-2 flex shrink-0 items-center gap-2 text-xs opacity-80">
              {/* badges go here, e.g. muted/live/host */}
              {!u.isLive && last && <span title={u.lastSeen}>Last seen {last}</span>}
            </span>
          </li>
        );
      })}
